/*
    배열(array)
      -배열 리터럴 방식으로 가장 많이 사용한다. (코드의 고정된 갑을 대표하는 용어)
      -배열의 총 크기를 배열의 길이라고한다.
        => 배열에 들어있는 요소의 개수와 배열의 길이는 항상 일치하지는 않는다.
        => 배열.length 프로퍼티를 통해 배열의 길이를 알수 있다.
...
// => 배열의 길이를 변화시키기 때문에 주의해야한다. 특히 반복문의 조건으로 사용할때
let numbersLength = numbers.push(0,43,54,44);
...
//문제 ) 배열의 모든 요소를 제거해보기. => 배열의 모든 요소가 사라질 때 까지 반복
...
// 첫번째 인자 : 시작 인덱스. undefined 의 경우 0으로 지정된다.
// 두번째 인자 : 종료 인덱스. undefined 의 경우 끝까지 복사한다. 종료 인덱스 요소는 포함되지 않는다.
let copyFruits = fruits.slice(-2);
console.log(copyFruits, fruits);

/*
   Array.prototype.splice : 배열의 중간에 요소를 제거하거나 추가할수있다. 원본 배열 변경이 된다.
      => 첫번째 인자 : 시작 인덱스
      => 두번째 인자 : 삭제할 개수.
      => 제거한 요소들을 배열로 반환한다 (잘라넣기 효과)
      => 중간의 요소들을 제거할때 사용한다. 혹은 복사만 하는게 아니라 아예 옮길때 사용한다. 
...
/*
   Array.prototype.indexof : 인자로 전달된 값의 인덱스 값을 반환
      => 찾는 요소가 없을경우 -1를 반환한다 => 찾는 요소가 없는 경우에 대한 분기처리를 할수있다.
*/
...
//문제 ) colors 에서 tomato 찾아보고 없으면 "찾는 색상이 없습니다" 라는 문구 출력하기
...
Array.prototype.includes : 배열에 원소가 있는지 true, false 반환 한다.
   => 나중에 추가된 메서드다.
   => 요소가 있는지 없는지 확인할 때는 includes를, 인덱스를 확인할때는 indexof 

*/

let isInclude = colors.includes('black');
if(!isInclude){
    console.log("찾는색상이 없습니다");}


    /*
        배열의 고차함수 (Higher Order Function)
               - 고차 함수란 함수를 인자로 전달받거나 함수를 반환하는 함수
    */


// 문제 ))arr의 각 요소에 *2 해서 출력하기 
let arr = [1,2,3,4];
...
/*
   Array.prototype.forEach
       ...
       =>각 요소를 참조하여 함수를 실행할수 있다.
       =>콜백 함수의 첫번째 인자로 각 요소가 한번씩 전달된다.
