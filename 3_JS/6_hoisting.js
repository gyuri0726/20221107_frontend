/*
    var의 문제점
    1. 호이스팅이 일어난다.
       - 실행 전에 오류를 내면 예측할수 있어서 수정이 용이하다
       - 호이스팅은 실제 실행될 때 오류를 발생시킬수 있어 수정하기 어렵다.
    2. 재선언이 가능하다.
       -다른 곳에서 사용중인 변수의 값을 의도치 않게 변경 할수있다.  => 큰일난다 ㅠㅠ
...
/*호이스팅(Hoisting): 끌어 올린다라는 뜻. 변수, 함수, 클래스 선언부를 위로 끌어올린 것처럼 동작한다.
     => 선언부만 끌어올린다. => undefind로 초기화된다.
*/
console.log(num);
var num = 10;
var num =1000;
...
//let 키워드를 사용하면 재 선언이 불가능하다. => 이미 선언된 식별자라면 오류를 발생시킨다.
let price = 1000;
//let price = 10000;


//const -상수(Constant)는 변하지 않는 수. = 재할당을 하면 오류가 난다. 변하면 안되는 값을 할당을 한다.
//  => 상수 선언시 값을 초기화 하지 않으면 오류가 발생.
const TAX_FATIO = 10;
//TAX_FATIO = 100;
...
// (((((((((((((((var 말고 let을써      그리고 변하지않아야되는거에는 const를 써 )))))))))))))))
...
//console.log(str); > 오류 (let은 블록{} 밖에 적용 안되니까)
